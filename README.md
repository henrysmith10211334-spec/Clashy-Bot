...
- 📹 Posts an embed when a new video (or Short) is uploaded
- 🔴 Posts a separate "LIVE NOW" embed the moment the channel starts streaming,
  and won't double-post it as a regular video once the stream ends
...
3. In the project's "Variables" tab, add:
   ...
   - `CHECK_INTERVAL_MINUTES` = `5` (optional, defaults to 5) — how often to check for new uploads
   - `LIVE_CHECK_INTERVAL_MINUTES` = `2` (optional, defaults to 2) — how often to check if the channel is live (checked more frequently since "going live" is time-sensitive)
